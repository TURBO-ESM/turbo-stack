...
int main(int argc, char* argv[])
{
    amrex::Initialize(argc, argv);
    {
        ...
        turbo::CartesianDomain domain(x_min, x_max, y_min, y_max, z_min, z_max, n_cell_x, n_cell_y, n_cell_z);
        ...
        auto fields = domain.GetFields();  // fields is a non owning view so it will automatically update when we add
                                           // fields to the domain

        // Now have access to geometry and grid information if needed... TODO: move this print functionality grid and
        // field into an overload of operator<< later
        ...
        amrex::Print() << "  Number of Cells: (" << grid->NCellI() << ", " << grid->NCellJ() << ", " << grid->NCellK()
                       << ")" << std::endl;
        amrex::Print() << "  Number of Nodes: (" << grid->NNodeI() << ", " << grid->NNodeJ() << ", " << grid->NNodeK()
                       << ")" << std::endl;
        amrex::Print() << std::endl;
        ...
        std::size_t n_component_vector = 3;
        {
            ...
            std::shared_ptr<turbo::Field> cell_scalar =
                domain.CreateField("cell_scalar", turbo::FieldGridStagger::CellCentered, n_component_scalar, n_ghost);
            std::shared_ptr<turbo::Field> cell_vector =
                domain.CreateField("cell_vector", turbo::FieldGridStagger::CellCentered, n_component_vector, n_ghost);

            std::shared_ptr<turbo::Field> node_scalar =
                domain.CreateField("node_scalar", turbo::FieldGridStagger::Nodal, n_component_scalar, n_ghost);
            std::shared_ptr<turbo::Field> node_vector =
                domain.CreateField("node_vector", turbo::FieldGridStagger::Nodal, n_component_vector, n_ghost);

            std::shared_ptr<turbo::Field> x_face_scalar =
                domain.CreateField("x_face_scalar", turbo::FieldGridStagger::IFace, n_component_scalar, n_ghost);
            std::shared_ptr<turbo::Field> x_face_vector =
                domain.CreateField("x_face_vector", turbo::FieldGridStagger::IFace, n_component_vector, n_ghost);

            std::shared_ptr<turbo::Field> y_face_scalar =
                domain.CreateField("y_face_scalar", turbo::FieldGridStagger::JFace, n_component_scalar, n_ghost);
            std::shared_ptr<turbo::Field> y_face_vector =
                domain.CreateField("y_face_vector", turbo::FieldGridStagger::JFace, n_component_vector, n_ghost);

            std::shared_ptr<turbo::Field> z_face_scalar =
                domain.CreateField("z_face_scalar", turbo::FieldGridStagger::KFace, n_component_scalar, n_ghost);
            std::shared_ptr<turbo::Field> z_face_vector =
                domain.CreateField("z_face_vector", turbo::FieldGridStagger::KFace, n_component_vector, n_ghost);
            ...
            for (const auto& field : {cell_scalar, cell_vector, node_scalar, node_vector, x_face_scalar, x_face_vector,
                                      y_face_scalar, y_face_vector, z_face_scalar, z_face_vector})
            {
        ...
        auto scalar_fields = std::views::filter(fields, [n_component_scalar](const FieldPtr& field)
                                                { return field->multifab->nComp() == n_component_scalar; });
        auto vector_fields = std::views::filter(fields, [n_component_vector](const FieldPtr& field)
                                                { return field->multifab->nComp() == n_component_vector; });
        auto nodal_fields  = std::views::filter(fields, [](const FieldPtr& field) { return field->IsNodal(); });
        auto cell_centered_fields =
            std::views::filter(fields, [](const FieldPtr& field) { return field->IsCellCentered(); });
        ...
        amrex::Print() << "Number of cell centered fields: " << std::ranges::distance(cell_centered_fields)
                       << std::endl;
        ...
        /////////////////////////////////////////////////////////////////////////////////////////////////
        //  Initialize all the scalar and vector MultiFabs in the Domain - Alternative approach without using
        //  Field::Initialize
        /////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            for (amrex::MFIter mfi(mf); mfi.isValid(); ++mfi)
            {
                ...
                amrex::ParallelFor(mfi.validbox(),
                                   [=] AMREX_GPU_DEVICE(int i, int j, int k)
                                   {
                                       ...
                                       array(i, j, k) =
                                           grid_point.x;  // Example: initialize scalar field with x-coordinate
                                   });
            }
        }
            ...
            for (amrex::MFIter mfi(mf); mfi.isValid(); ++mfi)
            {
                ...
                amrex::ParallelFor(
                    ...
                    [=] AMREX_GPU_DEVICE(int i, int j, int k)
                    {
                        const turbo::Grid::Point grid_point = field->GetGridPoint(i, j, k);
                        array(i, j, k, 0) =
                            grid_point.x;  // Example: initialize vector field component 0 with x-coordinate
                        array(i, j, k, 1) =
                            grid_point.y;  // Example: initialize vector field component 1 with y-coordinate
                        array(i, j, k, 2) =
                            grid_point.z;  // Example: initialize vector field component 2 with z-coordinate
                    });
            }
        }
