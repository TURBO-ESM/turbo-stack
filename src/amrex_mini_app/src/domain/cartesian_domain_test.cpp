...
class CartesianDomainTest : public ::testing::Test
{
   protected:
    ...
    void SetUp() override
    {
        ...
        cartesian_domain =
            std::make_unique<CartesianDomain>(x_min, x_max, y_min, y_max, z_min, z_max, n_cell_x, n_cell_y, n_cell_z);
    }
};
...
TEST(CartesianDomainConstructorTest, Constructor)
{
    ...
    CartesianDomain cartesian_domain(x_min, x_max, y_min, y_max, z_min, z_max, n_cell_x, n_cell_y, n_cell_z);

    // Expect constructor to throw with invalid input

    // Invalid domain extents (min >= max)
    EXPECT_THROW(CartesianDomain(1.0, -1.0, y_min, y_max, z_min, z_max, n_cell_x, n_cell_y, n_cell_z),
                 std::invalid_argument);

    EXPECT_THROW(CartesianDomain(x_min, x_max, 1.0, -1.0, z_min, z_max, n_cell_x, n_cell_y, n_cell_z),
                 std::invalid_argument);

    EXPECT_THROW(CartesianDomain(x_min, x_max, y_min, y_max, 1.0, -1.0, n_cell_x, n_cell_y, n_cell_z),
                 std::invalid_argument);

    // Invalid number of cells (zero)
    EXPECT_THROW(CartesianDomain(x_min, x_max, y_min, y_max, z_min, z_max, 0, n_cell_y, n_cell_z),
                 std::invalid_argument);

    EXPECT_THROW(CartesianDomain(x_min, x_max, y_min, y_max, z_min, z_max, n_cell_x, 0, n_cell_z),
                 std::invalid_argument);

    EXPECT_THROW(CartesianDomain(x_min, x_max, y_min, y_max, z_min, z_max, n_cell_x, n_cell_y, 0),
                 std::invalid_argument);
}
...
TEST_F(CartesianDomainTest, CreateAndGetField)
{
    ...
    EXPECT_THROW(cartesian_domain->CreateField(field_name, FieldGridStagger::CellCentered, n_component, n_ghost),
                 std::invalid_argument);
}
