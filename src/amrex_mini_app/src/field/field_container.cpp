...
std::shared_ptr<Field> FieldContainer::Insert(const Field::NameType& name, const FieldGridStagger field_grid_stagger, const std::size_t n_component, const std::size_t n_ghost) {
    if (Contains(name, field_grid_stagger)) {
        throw std::invalid_argument("FieldContainer::Insert: Field with name '" + name + "' and stagger '" + FieldGridStaggerToString(field_grid_stagger) + "' already exists.");
    }
    ...
    if (!inserted) {
        // Since we already checked that no value with this same key exist in the map, we should never reach this point. The insert should always succeed.
        throw std::logic_error("FieldContainer::Insert: Failed to insert field. Somehow it was not inserted into the map used under the hood of field container. Should never happen.");
    }
...
std::shared_ptr<Field> FieldContainer::Get(const Field::NameType& name, const FieldGridStagger stagger) const {
    ...
    throw std::invalid_argument("FieldContainer::Get: Field with name '" + name + "' and stagger '" + FieldGridStaggerToString(stagger) + "' does not exist.");
}
