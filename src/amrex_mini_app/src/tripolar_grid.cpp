...
{
    static_assert(amrex::SpaceDim == 3, "Only supports 3D grids.");

    // Initialize the MultiFabs

    // number of ghost cells
    const int n_ghost = 1;  // Maybe we dont want ghost elements for some of the MultiFabs, or only in certain
                            // directions, but I just setting it the same for all of them for now.
    ...
    {
        // We don't really need the AMREX_D_DECL here since we are enforcing 3D grids only, but it's fine to keep it for
        // now to avoid potential issues if we decide to extend to 2D or 1D later. If or when we do that we will need to
        // wrap all the change the other IntVect definitions accordingly.
    ...
    // All subsequent MultiFabs box arrays will be adjusted accordingly using convert and the box array from the
    ...
    // Create MultiFabs for scalar and vector fields on the x-face-centered grid
    {
        ...
        x_face_scalar =
            std::make_shared<amrex::MultiFab>(x_face_box_array, distribution_mapping, n_comp_scalar, n_ghost);
        x_face_vector =
            std::make_shared<amrex::MultiFab>(x_face_box_array, distribution_mapping, n_comp_vector, n_ghost);
    }
    ...
    // Create MultiFabs for scalar and vector fields on the y-face-centered grid
    {
        ...
        y_face_scalar =
            std::make_shared<amrex::MultiFab>(y_face_box_array, distribution_mapping, n_comp_scalar, n_ghost);
        y_face_vector =
            std::make_shared<amrex::MultiFab>(y_face_box_array, distribution_mapping, n_comp_vector, n_ghost);
    }
    ...
    // Create MultiFabs for scalar and vector fields on the z-face-centered grid
    {
        ...
        z_face_scalar =
            std::make_shared<amrex::MultiFab>(z_face_box_array, distribution_mapping, n_comp_scalar, n_ghost);
        z_face_vector =
            std::make_shared<amrex::MultiFab>(z_face_box_array, distribution_mapping, n_comp_vector, n_ghost);
    }
    ...
    all_multifabs = {cell_scalar,   cell_vector,   x_face_scalar, x_face_vector, y_face_scalar,
                     y_face_vector, z_face_scalar, z_face_vector, node_scalar,   node_vector};
...
{
    return {x_min_ + node_index[0] * dx_, y_min_ + node_index[1] * dy_, z_min_ + node_index[2] * dz_};
}
...
TripolarGrid::Point TripolarGrid::GetLocation(const std::shared_ptr<amrex::MultiFab>& mf, int i, int j, int k) const
{
    ...
    else
    {
        ...
        return {};  // Returned this line to silence the warning about control reaching end of non-void function. Will
                    // never be reached because we are calling abort in this case.
    }
}
...
void TripolarGrid::WriteHDF5(const std::string& filename) const
{
    ...
    if (amrex::ParallelDescriptor::MyProc() == 0)
    {
...

        {
            ...
            const hid_t attr_space_id = H5Screate(H5S_SCALAR);
            const hid_t attr_id =
                H5Acreate(file_id, "double_test_value", H5T_NATIVE_DOUBLE, attr_space_id, H5P_DEFAULT, H5P_DEFAULT);
...

        {
            ...
            const hid_t attr_space_id = H5Screate(H5S_SCALAR);
            const hid_t attr_id =
                H5Acreate(file_id, "int_test_value", H5T_NATIVE_INT, attr_space_id, H5P_DEFAULT, H5P_DEFAULT);
...
std::shared_ptr<amrex::MultiFab> TripolarGrid::CopyMultiFabToSingleRank(
    const std::shared_ptr<amrex::MultiFab>& source_mf, int dest_rank) const
{
    ...
    const amrex::DistributionMapping distribution_mapping(amrex::Vector<int>{
        dest_rank});  // Distribution mapping that puts the single box in the box array to a single rank
...
void TripolarGrid::WriteMultiFabsToHDF5(const hid_t file_id) const
{
    const std::map<std::string, std::shared_ptr<amrex::MultiFab>> name_to_multifab = {
        {"cell_scalar", cell_scalar},     {"cell_vector", cell_vector},     {"x_face_scalar", x_face_scalar},
        {"x_face_vector", x_face_vector}, {"y_face_scalar", y_face_scalar}, {"y_face_vector", y_face_vector},
        {"z_face_scalar", z_face_scalar}, {"z_face_vector", z_face_vector}, {"node_scalar", node_scalar},
    ...
    for (const auto& [name, src_mf] : name_to_multifab)
    {
        ...
        if (amrex::ParallelDescriptor::MyProc() == dest_rank)
        {
            ...
            amrex::Box box =
                mf->boxArray()[0];  // We are assuming that there is only one box in the MultiFabs box array.
            ...
            const hid_t dataset_id   = H5Dcreate(file_id, name.c_str(), H5T_NATIVE_DOUBLE, dataspace_id, H5P_DEFAULT,
                                                 H5P_DEFAULT, H5P_DEFAULT);

            std::vector<double> data(nx * ny * nz * n_component);

            // Iterate over the components of the MultiFab and fill the data vector... putting in row-major order
...
void TripolarGrid::WriteGeometryToHDF5(const hid_t file_id) const
{
    const std::map<std::string, std::shared_ptr<amrex::MultiFab>> geometry_name_to_a_multifab_at_that_location = {
        {"cell_center", cell_scalar}, {"x_face", x_face_scalar}, {"y_face", y_face_scalar},
        {"z_face", z_face_scalar},    {"node", node_scalar},
    };
...
//             {"cell_center", {nz, ny, nx}, "cell_center", "cell_scalar", "cell_vector", "Cell Scalar", "Cell Vector",
//             "Cell"},
//             {"node", {nz+1, ny+1, nx+1}, "node", "node_scalar", "node_vector", "Node Scalar", "Node Vector", "Node"},
//             {"x_face", {nz, ny, nx+1}, "x_face", "x_face_scalar", "x_face_vector", "X-Face Scalar", "X-Face Vector",
//             "Face"},
//             {"y_face", {nz, ny+1, nx}, "y_face", "y_face_scalar", "y_face_vector", "Y-Face Scalar", "Y-Face Vector",
//             "Face"},
//             {"z_face", {nz+1, ny, nx}, "z_face", "z_face_scalar", "z_face_vector", "Z-Face Scalar", "Z-Face Vector",
//             "Face"}
//         };
//
...
//
//             // Scalar field
//             xdmf << "      <Attribute Name=\"" << grid.scalar_name << "\" AttributeType=\"Scalar\" Center=\"" <<
//             grid.center << "\">\n"; WriteXDMFDataItem(grid.scalar_dataset, grid.dims, 1); xdmf << " </Attribute>\n";
//
//             // Vector field
//             xdmf << "      <Attribute Name=\"" << grid.vector_name << "\" AttributeType=\"Vector\" Center=\"" <<
//             grid.center << "\">\n"; WriteXDMFDataItem(grid.vector_dataset, grid.dims, 3); xdmf << " </Attribute>\n";
//
//             xdmf << "    </Grid>\n";
//         }
//
