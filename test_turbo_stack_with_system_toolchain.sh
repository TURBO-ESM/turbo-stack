#!/bin/bash
# test_turbo_stack_with_system_toolchain.sh
#
# End-to-end test driver (local / bring-your-own toolchain).  Thin wrapper: it
# parses the common driver args, then hands off to turbo_run_test_driver
# (scripts/lib/common.sh), which builds + ctests each selected backend (FMS2,
# TIM) by running the real single-backend builder
# scripts/build_local_with_system_toolchain.sh in its own process -- a full,
# independent Stage-1 (env setup) -> Stage-2 (build turbo-stack) run per backend
# -- and prints a per-backend matrix/verdict.  Its siblings,
# test_turbo_stack_locally.sh (spack toolchain) and test_turbo_stack_on_derecho.sh
# (Lmod modules), are identical except for their toolchain and builder.  See
# docs/build_test_orchestration_prompt.md and docs/dependency_tiers_prompt.md.
#
# Unlike the spack driver, this one installs NOTHING for you: your compiler /
# MPI / NetCDF / CMake must already be on PATH (system packages, Homebrew, an OS
# module system, or an already-activated Spack/Conda env).  The builder verifies
# the toolchain via setup_environment/local_toolchain_on_path.sh, then builds all
# upstream deps (pFUnit/AMReX + FMS/TIM) from the submodules.
#
...
#   (toolchain)                  Your compiler / MPI / NetCDF / CMake must be on
#                                PATH before running; this driver installs nothing.
...
turbo_run_test_driver "$TURBO_STACK_ROOT/scripts/build_local_with_system_toolchain.sh"
